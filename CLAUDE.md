...
- **Always-Loaded Memory Layer**: `MEMORY.md` now includes `## Critical Facts`, `## User Preferences`, and `## Active Projects`. New sessions preload these sections from `memory-auto-recall.ts` with a 5-minute cache TTL so the model starts with core context from the first message.
