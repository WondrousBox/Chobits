...
- [WondrousBox/Chobits](https://github.com/WondrousBox/Chobits) —— 本プロジェクトの前身。デスクトップマスコットの基礎形態と中核的なインタラクションはここから発展しました。
- [Open-LLM-VTuber](https://github.com/Open-LLM-VTuber/Open-LLM-VTuber) —— Live2D アバターのアプローチの源流であり、音声インタラクションパイプライン（ASR → LLM → TTS）と sherpa-onnx のモデル選定における重要な参考でもあります。
