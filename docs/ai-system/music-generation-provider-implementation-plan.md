...
路径说明：本文早期 Phase 记录中的 `packages/workflow/nodes/*` 是私有 integrations 迁移前的历史落点；当前已落地的 AI 工作流节点和 helper 位于 `packages/workflow-integrations/src/nodes/ai/`。历史记录保留原路径以便对照，不代表当前可用导入路径。
...
- `packages/workflow-integrations/src/nodes/ai/image-generate.ts` 已经展示了“能力筛选 + 动态模型选择 + execution service 调用”的可复用模式。
