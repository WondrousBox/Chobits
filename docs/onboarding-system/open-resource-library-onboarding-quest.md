# Quest: `open-resource-library` — 引导用户从助手右键菜单打开背包
...
用户希望新增一个“右键点击助手打开背包”的新手引导任务，并要求它出现在任务列表里。

这条任务用于教用户发现桌面助手的右键菜单，以及背包入口。它不是开机强制任务，不调用 LLM 生成流程；必须通过任务列表或未来 AI 显式触发。
...
4. 角色提示：“右键点我，打开菜单里的背包。”
5. 用户右键助手，菜单打开。
6. 角色提示：“现在点菜单里的「背包」。”
7. 用户点击助手右键菜单里的“背包”。
8. 菜单项派发 `ASSISTANT_MENU_ITEM_SELECTED`，payload 标记 `itemId: 'inventory'`、`windowKey: 'inventory'`、`source: 'assistant-context-menu'`。
9. QuestEngine 标记任务完成并发放奖励；routine 清理提示、播放庆祝反馈并说明背包用途。
...
- `recommendation`: 不配置；打开背包后不再继续推荐预览资源或围绕资源聊天
- `triggerEvents`: `ASSISTANT_MENU_ITEM_SELECTED`
- `completion`: `ASSISTANT_MENU_ITEM_SELECTED` 且 `itemId === 'inventory'`、`windowKey === 'inventory'`、`source === 'assistant-context-menu'`
...
注意：不能只用 `inventory` 窗口打开作为完成条件。AI、快捷入口或其他页面也可能打开背包；本任务必须确认用户学会了“右键助手 → 菜单 → 背包”的路径。
...
右键菜单的“背包”项点击时会先派发 purpose/app event，再打开 `inventory` 窗口。QuestEngine 只消费这个带业务来源的事件，不把其他打开背包的路径误算为完成。
...
- ✅ 右键助手后，routine 会提示点击菜单里的“背包”。
- ✅ 只有从助手右键菜单点击“背包”才完成任务。
- ✅ AI 或其他入口打开背包不会误完成任务。
