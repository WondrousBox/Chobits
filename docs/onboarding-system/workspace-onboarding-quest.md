...
- **窗口陪同讲解**：点击创建后先清掉邀请 notice，避免按钮气泡压住说明文字；`await-wizard-result` 开始等待后，角色说工作空间用途和快速创建提示。这两段话有 5 分钟冷却，窗口持续打开时不会每秒重复，但关闭未创建后仍会继续回到强制创建提示。
...
'onboarding.workspace.create.quickstart-tip': '快速开始会默认创建到文档文件夹。',
