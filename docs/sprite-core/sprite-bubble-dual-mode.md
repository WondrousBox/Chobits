...
│    ├── sprite:bubble:set-visible → show/hide           │
│    ├── sprite:config:get-bubble-mode                    │
│    └── sprite:config:set-bubble-mode + hide stale win   │
└───────────────────────────────────────────────────────┘
...
│    └── window['sprite:size:set']({ padding })         │
└───────────────────────────────────────────────────────┘
...
SpriteManager.showToast/showNotice/showBusy
  → sendRendererMessage(payload)
    → forwardBridgeMessage({ kind: 'show', payload, source: 'sprite' })
...
2. 节流后调用 `window.chobits.sprite.bubbleResize(width, height)`
...
- 切换模式 → `sprite:config:set-bubble-mode` handler 清空旧气泡窗口消息并隐藏非当前承载窗口
...
5. 主窗口收到 `sprite:config` → `effectivePadding` 变化 → 重新调用 `sprite:size:set` 调整窗口尺寸
