...
function setup(): IpcTestContext {
  ...
  const application = {
    ...
    execute: vi.fn().mockResolvedValue({ ok: true, runId: 'run-1', status: 'completed' }),
...
describe('workflow IPC adapter', () => {
  ...
  it('rejects malformed run requests before reaching the application service', async () => {
    ...
    await expect(context.invoke('wf:run', { definitionId: '' })).resolves.toMatchObject({ ok: false, error: 'invalid-run-request' });
    expect(context.application.execute).not.toHaveBeenCalled();
  });

  it('forwards valid run requests and preserves the IPC result contract', async () => {
    const context = setup();

    await expect(context.invoke('wf:run', { definitionId: 'workflow-1', input: { text: 'hello' }, scope: { kind: 'workspace', id: 'workspace-1' }, context: { source: 'test' } })).resolves.toEqual({
      ok: true,
      runId: 'run-1',
      status: 'completed'
    });
    expect(context.application.execute).toHaveBeenCalledWith({
    ...
    vi.mocked(context.application.execute).mockResolvedValueOnce({ ok: false, runId: 'run-2', status: 'failed', error: 'node failed' });
    await expect(context.invoke('wf:run', { definitionId: 'workflow-1' })).resolves.toMatchObject({ ok: false, runId: 'run-2', status: 'failed', error: 'node failed' });
  });
  ...
  it('keeps save request parsing at the IPC boundary', async () => {
    ...
    await expect(context.invoke('wf:saveDefinition', { definition: workflow, workspaceId: 'workspace-1' })).resolves.toEqual({ ok: true });
