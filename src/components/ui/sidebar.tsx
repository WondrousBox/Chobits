...
>(({ className, asChild = false, shouldShowOnHover = false, ...props }, ref) => {
  const Comp = asChild ? Slot : 'button';
...
>(({ className, shouldShowIcon = false, ...props }, ref) => {
  // Random width between 50 to 90%.
  const width = React.useMemo(() => {
    // eslint-disable-next-line react-hooks/purity -- shadcn 上游写法:skeleton 每次挂载取随机宽度
    return `${Math.floor(Math.random() * 40) + 50}%`;
  }, []);

  return (
    <div ref={ref} data-sidebar="menu-skeleton" className={cn('flex h-8 items-center gap-2 rounded-md px-2', className)} {...props}>
      {shouldShowIcon && <Skeleton className="size-4 rounded-md" data-sidebar="menu-skeleton-icon" />}
      <Skeleton
