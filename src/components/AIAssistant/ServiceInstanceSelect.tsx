          ...
          <div className="max-h-60 overflow-auto">
            {filteredResults.length === 0 ? (
              ...
              filteredResults.map(({ p, it }) => (
                <DropdownMenuItem key={`${p.id}:${it.id}`} onSelect={() => onChange(p.id, it.id)}>
              ...
              <DropdownMenuSub key={p.id}>
                <DropdownMenuSubTrigger className="no-drag">
                  <span className="flex items-center gap-2">
                    {p?.schema?.icon && <TintableSvg src={p.schema.icon} className="w-4 h-4" alt={p.label} />}
                    <span>{p.label}</span>
                  </span>
                ...
                <DropdownMenuSubContent className="no-drag">
                  ...
                  <DropdownMenuItem
                    ...
                    onSelect={async () => {
                      try {
                        await window.YUA.window['window:open']('settings' as any, { category: 'ai', aiProviderId: p.id });
                      } catch {
                        /* ignore */
                      }
                    }}
