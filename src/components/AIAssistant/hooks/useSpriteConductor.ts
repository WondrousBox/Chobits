...
function pickByEvent(anims: SpriteAnimation[], event: SpriteEventType): SpriteAnimation | undefined {
  return anims.find(a => a.meta.eventType === event)
}

function resolveEventsForState(state: AssistantVisualState): SpriteEventType[] {
  switch (state) {
    case 'idle': return ['idle', 'breath', 'idle2']
    case 'dragging': return ['drag', 'hold']
    case 'walking': return ['walk']
    case 'running': return ['run', 'walk']
    case 'click': return ['click']
    case 'hold': return ['hold', 'drag']
    case 'drop': return ['drop']
  }
}

function findAnimationForState(anims: SpriteAnimation[], state: AssistantVisualState): SpriteAnimation | undefined {
  const events = resolveEventsForState(state)
  for (const e of events) {
    const found = pickByEvent(anims, e)
    if (found) return found
  }
  return undefined
}

function preloadSprite(anim?: SpriteAnimation) {
  if (!anim) return
  ...
  setTimeout(() => { try { v.src = '' } catch { /* noop */ } }, 5000)
}
...
export default function useSpriteConductor() {
  ...
  const pick = useCallback((s: AssistantVisualState) => findAnimationForState(anims, s), [anims])

  const switchTo = useCallback((s: AssistantVisualState) => {
  ...
  const to = useCallback((s: AssistantVisualState) => {
  ...
  const playOnce = useCallback((s: AssistantVisualState, opts?: { fallback?: AssistantVisualState; durationMs?: number }) => {
    const anim = pick(s)
    if (!anim) return
