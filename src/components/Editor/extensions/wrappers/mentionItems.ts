...
export type PromptHandler = (message: string) => string | null;

const defaultPrompt: PromptHandler = (message) => {
 ...
 * 用于资源列表的简化 Mention 项
 */
export const createResourceMentionItems = (promptHandler: PromptHandler = defaultPrompt): MentionItem[] => [
  {
    value: 'timestamp',
    label: '⏰ 时间戳',
    onSelect: (editor, range) => {
  ...
  },
  {
    ...
    label: '🏷️ 标签',
    onSelect: (editor, range) => {
      const tag = promptHandler('输入标签名称：');
      if (tag) {
  ...
  },
  {
    ...
    label: '🔗 链接',
    onSelect: (editor, range) => {
      const url = promptHandler('输入链接地址：');
      if (url) {
        const text = promptHandler('输入链接文本（可选）：') || url;
  ...
  },
  {
    ...
    onSelect: (editor, range) => {
      ...
      const choice = promptHandler(`选择优先级：\n1. ${priorities[0]}\n2. ${priorities[1]}\n3. ${priorities[2]}\n\n请输入数字 1-3：`);
  ...
  },
  {
    ...
    onSelect: (editor, range) => {
      ...
      const choice = promptHandler(`选择状态：\n1. ${statuses[0]}\n2. ${statuses[1]}\n3. ${statuses[2]}\n4. ${statuses[3]}\n5. ${statuses[4]}\n\n请输入数字 1-5：`);
      const index = parseInt(choice || '', 10) - 1;
...
export const resourceMentionItems: MentionItem[] = createResourceMentionItems();
