...
function DragAbleTitle({ title, icon, actions, onClose }: DragAbleTitleProps) {
  ...
  useEffect(() => {
    ...
    if (window.YUA.isWindows) {
      window.YUA.window["window-is-maximized"]().then(v => { if (mounted) setMaximized(!!v) })
      window.YUA.window["window-capabilities"]().then(c => { if (mounted) setCaps(c) })
      const listener = (_: any, state: boolean) => { if (mounted) setMaximized(state) }
      window.ipcRenderer.on('window-maximize-changed', listener)
      return () => { mounted = false; window.ipcRenderer.off('window-maximize-changed', listener) }
    }
  ...
  return (
    <div className="flex items-center w-full drag-region gap-2 h-12 px-2 box-border bg-muted">
        ...
        <div className="no-drag flex items-center ml-2 -mr-2 select-none">
          ...
          <Button
            ...
            onClick={async () => {
              ...
              const r = await window.YUA.window["window-maximize-or-restore"]();
          ...
          <Button
            ...
            variant={"outline"}
            onClick={() => {
              if (onClose) onClose(); else window.YUA.window["window-close-self"]();
            }}
          >
            <VscChromeClose />
          </Button>
      ...
      {onClose && !window.YUA.isWindows && <Button className="w-8 h-8" onClick={onClose}>Close</Button>}
    </div>
  )
}
