            ...
            <div className="relative h-full flex flex-col">
              ...
              <div className="flex items-center justify-between px-3 py-2 border-b bg-background shrink-0">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="ghost" size="sm" className="h-8" onClick={performValidate}>
                    <TbCheck className="w-4 h-4 mr-1" />
                    校验
                  </Button>
                  <Button variant="outline" size="sm" className="h-8" onClick={performSave} disabled={saving || isPresetWorkflow} title={isPresetWorkflow ? '预设工作流不允许修改' : ''}>
                    保存
                  </Button>
                  {startNodeInputMode === 'text' || startNodeInputMode === 'url' || startNodeInputMode === 'file' || startNodeInputMode === 'folder' ? (
                    <Button size="sm" className="h-8" disabled={!draft || running} onClick={handleRunClick}>
                      <TbPlayerPlay className="w-4 h-4 mr-1" />
                      试运行
                    </Button>
                  ) : (
                    <ResourceRunPopover disabled={!draft} running={running} onSelect={runWorkflowWithResource} />
                  )}
                </div>
              </div>
