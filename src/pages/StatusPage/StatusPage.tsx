        ...
        <div className='flex-1 pl-1'>
          {role && (
            <div className='text-sm flex items-center gap-4'>
              <span>{role.name} <span className='text-muted-foreground text-xs ml-1'>Lv.{role.level ?? '—'}</span></span>
              <div className='flex items-center gap-1'><TbHeartFilled color='red' size={16} /><span className='font-mono text-xs text-muted-foreground'>{role.favor ?? 0}</span></div>
            </div>
          )}
        </div>
        <Button
      ...
      <div className="w-full h-full p-2 overflow-auto border border-ring box-border bg-background" style={{ height: "calc(100% - 40px)" }}>
        ...
        {overview?.workspace && (
          <div className='text-sm'>
            <div>当前空间: <span className='font-mono'>{overview.workspace.name}</span></div>
