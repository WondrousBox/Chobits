  ...
  return (
    <div onClick={onSelect} className={cn('flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-colors hover:bg-accent/50', selected && 'bg-accent ring-1 ring-primary/30')}>
      ...
      <div className="flex-1 min-w-0">
        <div className="text-sm font-medium text-foreground">{t('chatEntry.item.title')}</div>
...
export const ChatEntryDetailContent: React.FC<{ state: ChatEntrySettingsState }> = ({ state }) => {
  ...
  return (
    <div className="space-y-2">
      <h3 className="text-base font-semibold text-foreground">{t('chatEntry.detail.title')}</h3>
      <p className="text-sm text-muted-foreground">{state.isEnabled ? t('chatEntry.detail.descriptionOn') : t('chatEntry.detail.descriptionOff')}</p>
    </div>
  );
};
