...
export const ChatEntryItem: React.FC<{
  ...
  <div onClick={onSelect} className={cn('flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-colors hover:bg-accent/50', selected && 'bg-accent ring-1 ring-primary/30')}>
    <div className={cn('flex h-10 w-10 items-center justify-center rounded-full shrink-0', state.enabled ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground')}>
    ...
    <div onClick={(event) => event.stopPropagation()}>
      <Switch checked={state.enabled} disabled={state.loading || state.pending} onCheckedChange={(checked) => void state.setEnabled(checked)} />
    </div>
  </div>
);
...
export const ChatEntryDetailContent: React.FC<{ state: ChatEntrySettingsState }> = ({ state }) => (
  <div className="space-y-2">
    ...
    <p className="text-sm text-muted-foreground">{state.enabled ? '双击桌面精灵会打开跟随精灵的小输入窗。' : '双击桌面精灵会打开完整聊天窗口。'}</p>
  </div>
);
