/**
 * BubbleModeSettings — 桌面精灵气泡/特效展示模式总开关
 *
 * - 'inline' 传统模式：气泡与角色特效渲染在主精灵窗口内，沿用 padding 撑出的空白区域。
 * - 'external' 跟随窗口模式：气泡由独立的 spriteBubble 跟随窗口承载，主窗口 padding
 *   在运行期被视为 0，角色特效由独立的 spriteEffect 窗口承载。
 * - 'fixed-top' 顶部悬浮模式：气泡由独立的 spriteBubbleFixedTop 窗口固定在主窗口上方展示，
 *   角色特效继续由独立的 spriteEffect 窗口居中跟随。
 *
...
const BUBBLE_MODE_OPTIONS: Array<{ value: SpriteBubbleMode; label: string; description: string }> = [
  { value: 'inline', label: '传统（精灵内嵌）', description: '气泡和角色特效显示在主精灵窗口内，使用 padding 撑出的空白区域。' },
  { value: 'external', label: '跟随窗口', description: '气泡与角色特效使用独立窗口展示，主窗口 padding 运行期视为 0。' },
  { value: 'fixed-top', label: '顶部悬浮', description: '气泡固定在主窗口上方，角色特效使用独立窗口居中跟随。' }
];
  ...
  return (
    <div className={className}>
      <div className="flex items-center gap-2">
        <span className="text-xs text-muted-foreground whitespace-nowrap">气泡/特效模式</span>
