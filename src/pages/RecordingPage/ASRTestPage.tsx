...
import { TbLoader2, TbMicrophone, TbPlayerStop } from 'react-icons/tb';
...
const ASRTestPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const payload = (await window.YUA.window['window:payload:get']('asrTest' as any)) as { model?: string; language?: string } | undefined;
        if (!mounted) return;
  ...
  const handleOpenConfig = (): void => {
    ...
    window.YUA.window['window:close']('asrTest' as any);
  };
  ...
  return (
    <div className="flex flex-col h-full w-full box-border rounded-lg bg-background drag-region">
      ...
      <div className="flex gap-2 border-t p-2 px-4">
        <Button variant="outline" className="flex-1 no-drag" onClick={() => window.YUA.window['window:close']('asrTest' as any)}>
          关闭
        </Button>
        {isTesting ? (
