    ...
    <div>
      ...
      >
        ...
        <div className='flex items-center gap-1 opacity-80'>
          <Button size='icon' variant='ghost' className='w-8 h-8' onClick={(e) => { e.stopPropagation(); onRename(node.id) }}>
            <TbPencil />
          </Button>
          <Button size='icon' variant='ghost' className='w-8 h-8' onClick={(e) => { e.stopPropagation(); onDelete(node.id) }}>
            <TbTrash />
          </Button>
        </div>
      </div>
  ...
  return (
    <div className='h-full w-60 border-r flex flex-col bg-background'>
      <div className='p-2 flex items-center justify-between border-b'>
        ...
        <Button size='icon' className='w-8 h-8' onClick={onCreate}><TbPlus /></Button>
      </div>
      <div className='p-2'>
        <div
      ...
      <div className='flex-1 overflow-auto p-1'>
        ...
        {tree.length === 0 && (
          <div className='text-xs text-muted-foreground px-2'>暂无文件夹，点击右上角 + 新建</div>
        )}
      </div>
    </div>
  )
