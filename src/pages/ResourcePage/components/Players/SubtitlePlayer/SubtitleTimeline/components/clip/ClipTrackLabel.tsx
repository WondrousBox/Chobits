  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-0.5">
        <Button variant={activeTool === 'select' ? 'secondary' : 'ghost'} size="sm" className="w-6 h-5 p-0" onClick={() => onToolChange?.('select')} title={labels.selectTool}>
          <TbPointer className="w-3 h-3" />
        </Button>
        <Button variant={activeTool === 'cut' ? 'secondary' : 'ghost'} size="sm" className="w-6 h-5 p-0" onClick={() => onToolChange?.('cut')} title={labels.cutTool}>
