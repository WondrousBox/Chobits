...
type ReminderFormState = {
  ...
  date: string;
  time: string;
  repeat: CustomReminderInput['repeat'];
  leadMinutes: string;
  message: string;
};
...
const repeatOptions: Array<{ label: string; value: CustomReminderInput['repeat'] }> = [
      ...
      <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-base font-semibold text-foreground">日常照理模式</p>
              <p className="text-sm text-muted-foreground">{loading ? '正在读取状态…' : snapshot?.enabled ? '已开启，精灵会主动照料你的日常节奏。' : '关闭状态，精灵将暂停日常提醒。'}</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <Switch checked={snapshot?.enabled ?? false} onCheckedChange={handleToggleGlobal} disabled={globalPending} />
            <Button variant="outline" size="sm" className="gap-1" onClick={fetchSnapshot} disabled={loading}>
          ...
          <form onSubmit={handleSubmitReminder} className="rounded-2xl border border-border bg-card p-5 shadow-sm">
            ...
            <div className="space-y-3">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <Select value={formState.repeat} onValueChange={(value: ReminderFormState['repeat']) => setFormState((prev) => ({ ...prev, repeat: value }))}>
                  <SelectTrigger>
