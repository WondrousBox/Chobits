    ...
    <div className="h-full flex">
      ...
      <div className="w-[220px] flex flex-col border-r border-border">
        <div className="p-3 flex items-center gap-2">
          <Input className="h-8 flex-1" placeholder={t('prompt.searchPlaceholder')} value={tmplSearch} onChange={(e) => setTmplSearch(e.target.value)} />
        ...
        <ScrollArea className="flex-1">
          <div className="px-2 pb-2">
            {filteredTemplates.length === 0 ? (
              <div className="text-xs text-muted-foreground text-center py-6">{templates.length === 0 ? t('prompt.noTemplates') : t('prompt.noResults')}</div>
            ) : (
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="px-4 py-3 border-b border-border flex items-center justify-between">
          <div className="text-sm font-medium text-foreground">{selected ? selected.name : t('prompt.noSelection')}</div>
          {selected && (
        ...
        <div className="flex-1 p-4 min-h-0">
          <Textarea
            ...
            placeholder={selected ? t('prompt.editorPlaceholder') : t('prompt.selectFirst')}
            value={draftContent}
      ...
      {/* 新建对话框 */}
      <PromptTemplateFormDialog isOpen={isFormOpen} mode={formMode} title={t('prompt.createTitle')} initialValues={formValues} onClose={() => setIsFormOpen(false)} onSubmit={submitForm} />
    </div>
  );
}
