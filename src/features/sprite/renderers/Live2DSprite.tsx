...
const MODEL_BASE_URL = 'res://local/characters/live2d/';
...
export default function Live2DSprite({ width, height, walkDirection }: { width?: number; height?: number; walkDirection?: 'left' | 'right' | null }): JSX.Element {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/set-state-in-effect -- 切换模型时同步重置运行时状态,避免旧模型残留渲染
  ...
  useEffect(() => {
    if (!isRuntimeReady || !currentAnimation) return;
  ...
  }, [currentAnimation, isRuntimeReady, config]);
  ...
  }, [isRuntimeReady, config]);

  // 视线追踪：将鼠标位置映射到 Live2D 视图坐标
  useEffect(() => {
    if (!isRuntimeReady || !config?.lookAt?.enabled) return;
