...
export function getFfmpegPath(binName: "ffmpeg" | "yt-dlp") {
  switch (binName) {
    case "ffmpeg":
      return getRealPath(
        `../ffmpeg/${os.platform() === "darwin" ? "ffmpeg" : "ffmpeg.exe"}`,
    ...
    default:
      return getRealPath(
        `../${binName}/${os.platform() === "darwin" ? "ffmpeg" : "ffmpeg.exe"}`,
        `./resources/${binName}/${os.platform()}/${os.arch()}/${os.platform() === "darwin" ? "ffmpeg" : "ffmpeg.exe"}`,
      );
  }
}