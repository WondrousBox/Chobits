...
export function initStatusHandlers(_win: BrowserWindow) {
  ...
  ipcMain.handle('status:getOverview', async () => {
    ...
    const docByType = getAll<{ docType: string | null; count: number }>(
      `SELECT doc_type as docType, COUNT(*) as count FROM documents WHERE deleted_at IS NULL GROUP BY doc_type`
    )
