...
export interface QuestResetProgressResult {
  resetQuestIds: string[];
  achievementIds: string[];
  rewardSources: string[];
}
...
export class QuestEngine {
  ...
  async resetCompleted(): Promise<QuestResetCompletedResult> {
    if (!this.loaded) await this.init();

    const resetQuestIds: string[] = [];
    const achievementIds = new Set<string>();
    const rewardSources = new Set<string>();

    for (const def of this.deps.registry.list()) {
      const runtime = this.state.quests[def.id];
      const achievementUnlocked = await this.isQuestAchievementUnlocked(def);
      if (runtime?.status !== 'done' && !achievementUnlocked) continue;

      delete this.state.quests[def.id];
      resetQuestIds.push(def.id);

      const achievementId = this.getQuestAchievementId(def);
      if (achievementId) {
        achievementIds.add(achievementId);
      }
      rewardSources.add(def.rewardSource ?? `quest:${def.id}`);
    }
    ...
    return {
      ...
      rewardSources: Array.from(rewardSources)
    };
  }

  async resetProgress(): Promise<QuestResetProgressResult> {
    if (!this.loaded) await this.init();

    const resetQuestIds = Object.keys(this.state.quests);
    const achievementIds = new Set<string>();
    const rewardSources = new Set<string>();
    ...
    for (const def of this.deps.registry.list()) {
      ...
      if ((await this.isQuestAchievementUnlocked(def)) && !resetQuestIds.includes(def.id)) {
        resetQuestIds.push(def.id);
      }
    }
