...
import type { NodeHandler, PortSchema } from '../types';
...
export const PaddleOCRNode: NodeHandler = {
  ...
  async run({ input, config: nodeConfig, ctx, emit }) {
    ...
    const result = await recognizeWithPaddleOcr(imagePath, ctx.pluginResourceManager, {
