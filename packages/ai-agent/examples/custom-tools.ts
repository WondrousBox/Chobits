...
const WeatherTool = createTool<{ city: string; unit?: 'celsius' | 'fahrenheit' }, { city: string; temperature: number; condition: string; unit: string }>({
    name: 'get_weather',
    ...
    async execute(params) {
        ...
        const weatherData: Record<string, { temp: number; condition: string }> = {
            ...
            杭州: { temp: 16, condition: '小雨' }
        };

        const cityData = weatherData[params.city];
        if (!cityData) {
            throw new Error(`未找到城市 "${params.city}" 的天气数据`);
        }
    ...
    for await (const event of runtime.run(
        {
            messages: [
                {
                    role: 'user',
                    content: '北京今天天气怎么样？然后帮我把"早上好"翻译成英文'
                }
            ]
        },
        {
