...
export function normalizeRequestedSkillInvocation(input: unknown): RequestedSkillInvocation | undefined {
  ...
  const remainingQuery = typeof (input as { remainingQuery?: unknown }).remainingQuery === 'string' ? (input as { remainingQuery: string }).remainingQuery.trim() : undefined;
  const rawSource = typeof (input as { source?: unknown }).source === 'string' ? (input as { source: string }).source.trim().toLowerCase() : '';
...
export function resolveRequestedSkillInvocation(invocation: RequestedSkillInvocation, registry: SkillRegistry): ExplicitSkillInvocation | undefined {
...
function toExplicitSkillInvocation(record: SkillRecord, matchedReference: string, remainingQuery?: string): ExplicitSkillInvocation {
