...
export async function executeSkill(entry: SkillRegistryEntry, options: SkillExecutionOptions = {}): Promise<SkillExecutionResult> {
  ...
  const activationToolNames = entry.record.activationToolIds.map((toolId) => getPiToolDescriptor(toolId)?.name).filter((toolName): toolName is string => Boolean(toolName));
