...
import {
  ...
  executeSkill,
  findSkillByReference,
  getSkillSourceInfo,
  getSkillSourcePolicy,
  isSkillPathMatched,
  markGuardedSkillApproved,
  requiresSkillSourceCaution
} from '../skills';
...
export function createPiSkillUseTool(toolContext: PiSessionToolContext): ToolDefinition<typeof skillUseParameters> {
  return {
    name: 'skillUseTool',
    label: 'skillUseTool',
    description: '按名称加载并使用一个 skill。会读取 skill 正文、返回约束与说明；inline 模式下还会激活该 skill 建议的工具，preview 模式只查看不激活。',
    ...
    async execute(_toolCallId, input) {
      ...
      if (mode === 'inline' && sourcePolicy.requiresExplicitUserIntent && !hasExplicitUserIntent && input.acknowledgeRisk !== true) {
        ...
        if (confirmationDecision === 'preview') {
          ...
          return createJsonToolResult({
            ...
            nextStep: sourcePolicy.requiresPreviewBeforeInline
      ...
      try {
        ...
        if (effectiveMode === 'inline' && sourcePolicy.riskLevel === 'guarded' && (input.acknowledgeRisk === true || confirmationOutcome === 'confirmed-inline')) {
        ...
        if (effectiveMode === 'inline' && result.executionContext === 'inline' && toolContext.session && result.activationToolIds.length > 0) {
          const suggestedToolNames = result.activationToolIds.map((toolId) => getPiToolDescriptor(toolId)?.name).filter((toolName): toolName is string => Boolean(toolName));
