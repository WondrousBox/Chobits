...
import { type NodeConfig, type NodeHandler, onAbort, type PortSchema, type ValueType } from '@chobits/workflow';
...
async function transcodeAudio(filePath: string, outputDir: string, signal?: AbortSignal): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    const cmd = ffmpeg(filePath).toFormat('wav').audioFrequency(16000).audioChannels(1).audioCodec('pcm_s16le');
...
export const TranscribeFastWhisperNode: NodeHandler = {
  ...
  execution: { group: 'local-asr' },
  async run({ input, config, ctx: executionContext, emit, capabilities }) {
    const _ctx = capabilities.require(WORKFLOW_LOCAL_PROCESSING).resolveContext(executionContext);
    ...
    if (_ctx.getResourceProjectDirs) {
      const dirs = await _ctx.getResourceProjectDirs('transcribe');
      if (dirs) {
        projectDirs = dirs;
      } else {
    ...
    if (projectDirs.isResource) {
      console.log(`[fast-whisper] 使用资源项目目录: resourceId=${projectDirs.resourceId}, temp=${projectDirs.tempDir}`);
    } else {
      console.log(`[fast-whisper] 使用传统目录: ${projectDirs.tempDir}`);
    }
