...
import type { NodeConfig, NodeHandler, PortSchema } from '@chobits/workflow';

import { WORKFLOW_RESOURCE_READ, WORKFLOW_RESOURCE_WRITE } from '../../capabilities/resources';
...
export const StartNode: NodeHandler = {
  ...
  async run({ input, config, ctx, capabilities }) {
    ...
    switch (mode) {
      ...
      case 'folder': {
        ...
        // 只有当输入中有值且上下文中没有时才更新
        if (inputFolderId && inputWorkspaceId && (!ctx.folderId || !ctx.workspaceId) && ctx.workflowRunId) {
          capabilities.get(WORKFLOW_RESOURCE_WRITE)?.updateContext(ctx.workflowRunId, { workspaceId, folderId });
        }
      ...
      default: {
        ...
        const resourceReader = capabilities.get(WORKFLOW_RESOURCE_READ);
        if (resourceId && resourceReader) {
          try {
            const fullResource = await resourceReader.resources.getById(resourceId);
            if (fullResource) {
