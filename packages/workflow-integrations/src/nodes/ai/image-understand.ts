...
import { WORKFLOW_AI } from '../../capabilities/ai';
import { getDynamicModelConfig, getWorkflowProviderPresetId } from './ai-workflow-utils';
...
export const ImageUnderstandNode: NodeHandler = {
  ...
  requiredCapabilities: [WORKFLOW_AI],
  execution: { group: 'ai' },
  async run({ input, config, ctx, emit, capabilities }) {
    const ai = capabilities.require(WORKFLOW_AI);
    ...
    const imageContent = ai.readImage(imagePath);
...
请严格按照以下JSON格式返回结果，不要包含任何其他文字说明：
{
  ...
  "tags": ["分类标签", "标签1", "标签2", "标签3"]
    ...
    const { text: resultText } = await ai.executeChat({
      emit,
      messages: [
        {
          role: 'user',
      ...
      providerPresetId,
      signal: ctx.signal,
      workflowAiUsage: ai.buildUsageContext(ctx, {
        nodeLabel: '图片理解',
