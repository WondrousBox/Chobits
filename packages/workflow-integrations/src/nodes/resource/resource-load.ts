...
import { WORKFLOW_RESOURCE_READ } from '../../capabilities/resources';

export const ResourceLoadNode = defineNode({
  spec: {
    id: 'resource/load',
    label: '加载资源',
    category: 'Resource',
  ...
  async run({ input, capabilities }) {
    const resourceId = String(input.resourceId || '').trim();
    if (!resourceId) throw new Error('缺少资源 ID');

    const resource = await capabilities.require(WORKFLOW_RESOURCE_READ).resources.getById(resourceId);
    if (!resource) throw new Error(`未找到 ID 为 ${resourceId} 的资源`);

    const ext = resource.filePath ? path.extname(String(resource.filePath)).toLowerCase() : '';
    return {
      resource,
